...
destroy_delay_ring()
{
  c_free(delay_indices.begin);
  delay_indices.begin = NULL;
  c_free(delay_ring.begin);
  delay_ring.begin = NULL;
  c_free(threads_to_use.begin);
  threads_to_use.begin = NULL;
}

bool
thread_done(size_t thread)
{
  bool ret;
  if (!parser_done())
    return false;
  ...
  ret
    = (delay_indices[thread] == local_index || global.delayed_global)
    && (!(global.backprop || global.delayed_global
          || global.corrective) || global.local_prediction
        <= 0 || (delay_indices[thread + 1
                 + global.num_threads()] == global_index
...
example*
get_delay_example(size_t thread)
{
  //semiblocking
  ...
  if ((global.backprop || global.corrective || global.delayed_global)
    ...
    if (local_index > delay_indices[thread + global_offset] + (global.ring_size
                                                               >> 1) || (parser_done() && (local_index
                                                                                           == delay_indices[thread] || global.delayed_global)))
      //We want to do global training
    {
      if (delay_indices[thread + global_offset] == global_index
          && !parser_done()) {//global update unavailable.
        pthread_mutex_lock(&delay);
  ...
  if (delay_indices[thread] != local_index && (!global.delayed_global
                                               || global.local_prediction <= 0))
...
example*
blocking_get_delay_example(size_t thread)
{
  size_t index = delay_indices[thread] % global.ring_size;
  pthread_mutex_lock(&delay);
...
void
delay_example(example* ex, size_t count)
{
  size_t delay_count = count + mesg;
  if ((global.backprop || global.corrective) && global.local_prediction > 0)
    delay_count += count;
...
void
delay_global_example(example* ex)
