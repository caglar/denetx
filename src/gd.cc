...
{
  if (global.sd->weighted_examples > global.sd->dump_interval && !global.quiet
    ...
    fprintf(
            ...
            global.sd->sum_loss_since_last_dump / (global.sd->weighted_examples
                                                   - global.sd->old_weighted_examples), global.sd->example_number,
            global.sd->weighted_examples, label_buf, ec->final_prediction,
...
query_decision(example* ec, float k)
{
  ...
  if (k <= 1.)
    ...
    weighted_queries = global.initial_t + global.sd->weighted_examples
      - global.sd->weighted_unlabeled_examples;
    avg_loss = global.sd->sum_loss / k + sqrt(
