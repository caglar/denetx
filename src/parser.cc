...
void
parse_cache(po::variables_map &vm, string source,
  ...
  if (ngram == 0 && gram_mask.last() < initial_length) {
    ...
    for (size_t i = 0; i < last; i++) {
      ...
      for (size_t n = 1; n < gram_mask.index(); n++)
        new_index += new_index * quadratic_constant +
...
{
  used_index = (uint64_t*) c_calloc(num_threads, sizeof(uint64_t));
  global.parsed_examples = 0;
  done = false;
  examples = (example*) c_calloc(global.ring_size, sizeof(example));
