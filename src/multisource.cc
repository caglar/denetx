...
int
really_read(int sock, void* in, size_t count)
{
    char* buf = (char*) in;
    size_t done = 0;
    int r = 0;
...
bool
blocking_get_prediction(int sock, prediction &p)
{
...
bool
blocking_get_global_prediction(int sock, global_prediction &p)
{
    int count = really_read(sock, &p, sizeof(p));
    bool ret = (count == sizeof(p));
    return ret;
...
void
send_prediction(int sock, prediction &p)
{
...
void
send_global_prediction(int sock, global_prediction p)
{
    if (write(sock, &p, sizeof(p)) < (int) sizeof(p)) {
...
int
receive_features(parser* p, void* ex)
{
    example* ae = (example*) ex;
    io_buf* input = p->input;
    fd_set fds;
    FD_ZERO(&fds);
    for (int* sock = input->files.begin; sock != input->files.end
            - num_finished; sock++)
        FD_SET(*sock,&fds);
    ...
    while (input->files.index() > num_finished) {
        ...
        for (int index = 0; index < (int) (input->files.index() - num_finished); index++) {
            ...
            if (FD_ISSET(sock, &fds)) {//there is a feature or label to read
                prediction pre;
                if (!blocking_get_prediction(sock, pre)) {
                    ...
                    if (pre.example_number != ++(p->counts[index]))
                        cout << "count is off! " << pre.example_number
                                << " != " << p->counts[index] << " for source "
                                << index << " prediction = " << pre.p << endl;
                    ...
                    if (p->pes[ring_index].example_number
                        ...
                        cerr << "Error, example "
                                << p->pes[ring_index].example_number << " != "
                                << pre.example_number << endl;
                    feature f =
                        { pre.p, p->ids[index] };
                    ...
                    if (p->pes[ring_index].features.index() == input->count) {
                        ...
                        push_many(ae->atomics[multindex],
                                p->pes[ring_index].features.begin,
                                p->pes[ring_index].features.index());
