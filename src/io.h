...
public:
    ...
    virtual int
    open_file(const char* name, int flag = READ)
    {
        int ret;
        switch (flag) {
        case READ:
    ...
    virtual ssize_t
    read_file(int f, void* buf, size_t nbytes)
    {
    ...
    virtual ssize_t
    write_file(int f, const void* buf, size_t nbytes)
    {
    ...
    flush()
    {
        if (write_file(files[0], space.begin, space.index())
                != (int) space.index())
...
void
buf_write(io_buf &o, char* &pointer, int n);

unsigned int
buf_read(io_buf &i, char* &pointer, int n);

bool
isbinary(io_buf &i);

size_t
readto(io_buf &i, char* &pointer, char terminal);
#endif
