...
void
quad_grad_update(weight* weights, feature& page_feature,
...
void
quad_precond_update(weight* weights, feature& page_feature,
        v_array<feature> &offer_features, size_t mask, float g)
{
    size_t halfhash = quadratic_constant * page_feature.weight_index;
    float update = g * page_feature.x;
...
float
predict_and_gradient(regressor& reg, example* &ec)
{
    ...
    for (vector<string>::iterator i = global.pairs.begin(); i
        ...
        if (ec->subsets[(int) (*i)[0]].index() > 0) {
            ...
            for (; temp.begin != temp.end; temp.begin++)
                quad_grad_update(weights, *temp.begin,
                        ec->atomics[(int) (*i)[1]], thread_mask, loss_grad);
        }
    }
    return fp;
}
...
void
update_preconditioner(regressor& reg, example* &ec)
{
    label_data* ld = (label_data*) ec->ld;
    float curvature = reg.loss->second_derivative(ec->final_prediction,
            ld->label) * ld->weight;
    ...
    for (vector<string>::iterator i = global.pairs.begin(); i
        ...
        if (ec->subsets[(int) (*i)[0]].index() > 0) {
            ...
            temp.end = ec->subsets[(int) (*i)[0]][1];
            for (; temp.begin != temp.end; temp.begin++)
                quad_precond_update(weights, *temp.begin,
                        ec->atomics[(int) (*i)[1]], thread_mask, curvature);
        }
    }
}

float
dot_with_direction(regressor& reg, example* &ec)
{
    float ret = 0;
    weight* weights = reg.weight_vectors[0];
    size_t thread_mask = global.thread_mask;
    ...
    for (vector<string>::iterator i = global.pairs.begin(); i
        ...
        if (ec->subsets[(int) (*i)[0]].index() > 0) {
            ...
            temp.end = ec->subsets[(int) (*i)[0]][1];
            for (; temp.begin != temp.end; temp.begin++)
                ret += one_pf_quad_predict(weights, *temp.begin,
...
double
derivative_magnitude(regressor& reg, float* old_first_derivative)
...
double
direction_magnitude(regressor& reg)
...
double
derivative_diff_mag(regressor& reg, float* old_first_derivative)
...
double
add_regularization(regressor& reg, float regularization)
{//compute the derivative difference
    double ret = 0.;
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;
...
void
finalize_preconditioner(regressor& reg, float regularization)
{
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;
    weight* weights = reg.weight_vectors[0];
...
void
zero_state(regressor& reg, float* old_first_derivative)
{
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;
    weight* weights = reg.weight_vectors[0];
...
double
derivative_in_direction(regressor& reg, float* old_first_derivative)
{
    double ret = 0.;
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;
...
void
update_direction(regressor& reg, float old_portion, float* old_first_derivative)
{
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;
    weight* weights = reg.weight_vectors[0];
    ...
    for (uint32_t i = 0; i < length; i++)
        weights[stride * i + 2] = old_first_derivative[i] * weights[stride * i
                + 3] + old_portion * weights[stride * i + 2];
}

void
update_weight(regressor& reg, float step_size)
{
    uint32_t length = 1 << global.num_bits;
    size_t stride = global.stride;

    for (uint32_t i = 0; i < length; i++)
        reg.weight_vectors[0][stride * i] += step_size
                * reg.weight_vectors[0][stride * i + 2];
}

void
setup_cg(gd_thread_params t)
{
    regressor reg = t.reg;
    size_t thread_num = 0;
    example* ec = NULL;
    ...
    float* old_first_derivative = (float*) c_malloc(
            sizeof(float) * global.length());

    if (!global.quiet) {
        const char * header_fmt = "%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\n";
        fprintf(stderr, header_fmt, "avg. loss", "mix fraction", "curvature",
                "dir. magnitude", "step size", "d_mag*step/examples");
        cerr.precision(5);
    }
    ...
    while (true) {
        if ((ec = get_example(thread_num)) != NULL)//semiblocking operation.
        {
            ...
            {
                if (current_pass == 0)
                    finalize_preconditioner(reg,
                            global.regularization * importance_weight_sum);
                if (gradient_pass) // We just finished computing all gradients
                {
                    if (global.regularization > 0.)
                        loss_sum += add_regularization(reg,
                                global.regularization * importance_weight_sum);
                    if (!global.quiet)
                        fprintf(stderr, "%-f\t",
                                loss_sum / importance_weight_sum);

                    if (current_pass > 0 && loss_sum > previous_loss_sum) {// we stepped to far last time, step back
                        if (ec->pass != 0)
                            step_size *= 0.5;
                        if (!global.quiet)
                        ...
                        float mix_frac = 0;
                        if (current_pass != 0)
                            mix_frac = derivative_diff_mag(reg,
                                    old_first_derivative) / previous_d_mag;
                ...
                else // just finished all second gradients
                {
                    ...
                    if (curvature == 0. && dd != 0.) {
                        cout
                                << "your curvature is 0, something wrong.  Try adding regularization"
                                << endl;
                        exit(1);
                    }
                    ...
                    if (!global.quiet)
                        fprintf(stderr, "%-e\t%-e\t%-e\t%-f\n",
                                curvature / importance_weight_sum, d_mag,
                                step_size,
                                d_mag * step_size / importance_weight_sum);
                    predictions.erase();
                    update_weight(reg, step_size);
                    gradient_pass = true;
                }
                ...
                if (ec->pass == 0)
                //new examples incoming, reset everything.
                {
                    zero_state(reg, old_first_derivative);
                    //set all except weights to 0.
            ...
            else //computing curvature
            {
                ...
                ec->loss = reg.loss->getLoss(ec->final_prediction, ld->label)
                        * ld->weight;
                float sd = reg.loss->second_derivative(
                        predictions[example_number++], ld->label);
