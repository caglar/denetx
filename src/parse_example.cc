...
void
feature_value(substring &s, v_array<substring>& name, float &v)
{
  tokenize(':', s, name);
...
template<class T>
void
copy_v_array(v_array<T>& old, v_array<T>& new_va)
{
  // allocate new memory for new.
  new_va.begin = (T *) c_malloc(sizeof(T) * old.index());

  // copy the old to the new.
  c_memcpy(new_va.begin, old.begin, sizeof(T) * old.index());
...
char*
c_string_of_substring(substring s)
{
  size_t len = s.end - s.start + 1;
  char* ret = (char *) c_calloc(len, sizeof(char));
  c_memcpy(ret, s.start, len - 1);
  return ret;
}
...
copy(char* base)
{
  ...
  while (base[len++] != '\0')
    ;
  char* ret = (char *) c_calloc(len, sizeof(char));
  c_memcpy(ret, base, len);
  return ret;
}

int
read_features(parser* p, void* ex)
{
  ...
  if (*line == '|')
    ...
    char* tab_location = safe_index(label_space.start, '\t',
                                    label_space.end);
    ...
    //        std::cout << "Label: " << ((label_data *)ae->ld)->label << std::endl;
  }
  ...
  for (substring* i = feature_start; i != p->channels.end; i++) {
    ...
    if (channel.start[0] != ' ') // We have a nonanonymous namespace
    {
      ...
      if (ae->atomics[index].begin == ae->atomics[index].end)
        new_index = true;
      if (audit) {
        base = (char *) c_calloc(2, sizeof(char));
        base[0] = ' ';
        base[1] = '\0';
      }
      channel_hash = 0;
    ...
    if (audit) {
      for (substring* i = p->words.begin + feature_offset; i
           != p->words.end; i++) {
        float v;
        feature_value(*i, p->name, v);
        v *= channel_v;
        ...
        audit_data ad =
        { copy(base), feature, word_hash, v, true };
