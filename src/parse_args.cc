           ...
           regressor &r, parser* par, string& final_regressor_name)
{
  ...
  desc.add_options()
    ...
    ("naive_bayes,nb", "naive bayes learning mode")
    ("nb_model", po::value<string>(), "naive bayes learning model")
    ("kernel_estimator,k", "use kernel estimator for naive bayes algorithm")
    ("hoeffding_tree,ht", "use hoeffding tree algorithm")
    ("arfx_path", po::value<string>(), "XML arf file's path")
    ...
    ("training_data", po::value<string>()->default_value(""), "Training data set for supervised learning.")
    ("data,d", po::value<string>()->default_value(""), "Example Set")
    ("daemon", "read data from port 39523")
    ("decay_learning_rate", po::value<float>(&global.eta_decay_rate)->default_value(default_decay), "Set Decay factor for learning_rate between passes")
    ...
    ("delayed_global", "Do delayed global updates")("hash", po::value<string>(), "how to hash the features. Available options: strings, all")
    ...
    ("initial_t", po::value<float>(&(global.sd->t))->default_value(1.), "initial t value")
    ...
    ("learning_rate,l",  po::value<float>(&global.eta)->default_value(10),            "Set Learning Rate")
    ("passes",            po::value<size_t>(&global.numpasses)->default_value(1),            "Number of Training Passes")
  ...
  if (vm.count("predictions")) {
    ...
    if (strcmp(vm["predictions"].as<string> ().c_str(), "stdout") == 0)
      push(global.final_prediction_sink, (size_t) 1);
    else {
      const char* fstr = (vm["predictions"].as<string> ().c_str());
      size_t f = fileno(fopen(fstr, "w"));
      if (f < 0)
